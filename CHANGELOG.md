...
### Fixed
- **Canonical `seoflow` npm name is live** — the package now publishes under both `seoflow` and the legacy `@imsankz/seoflow` (`npm run publish:both`); `npx seoflow` works for everyone.
- **URL audit now works from the npm-installed CLI** — the published package has no `python/` dir, so `npx seoflow audit <url>` silently fell back to HEAD-only signals (empty title/H1/schema/link analysis for every user). Added a pure-Node fetch fallback (Node 18+ `fetch` + regex parser) — zero Python deps, real signals.
