...
function getCitationsPaths() {
  const dataDir = path18.dirname(getAuditLogPath());
  const historyPath = loadConfig().citations?.historyPath ? path18.resolve(loadConfig().citations.historyPath) : path18.join(dataDir, "citations-history.json");
  return {
    historyPath,
    sovPath: path18.join(dataDir, "citations-sov.json")
  };
}
...
function readJson(p) {
  ...
  try {
    const raw = JSON.parse(fs19.readFileSync(p, "utf8"));
...
function writeJson(p, data) {
  fs19.mkdirSync(path19.dirname(p), { recursive: true });
  fs19.writeFileSync(p, JSON.stringify(data, null, 2));
}
...
function provisionExtensionBundle(extensionId, rootDir) {
  ...
  fs20.mkdirSync(path20.dirname(destinationDir), { recursive: true });
...
function writeState(rootDir, state) {
  const statePath = getStateFilePath(rootDir);
  fs20.mkdirSync(path20.dirname(statePath), { recursive: true });
      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.data) {
            ...
            fs21.mkdirSync(path21.dirname(sidecarPath), { recursive: true });
...
function findRoot3() {
  let dir = process.cwd();
  for (let i = 0; i < 10; i++) {
    if (fs22.existsSync(path22.join(dir, "seoflow.config.json"))) return dir;
...
\u274C File not found: ${inFile || "(no file specified)"}`);
  ...
  if (bundle.learning) {
    fs26.writeFileSync(path27.join(dataDir, "learning.json"), JSON.stringify(bundle.learning, null, 2));
    console.log("  \u2705 Imported learning.json");
  }
  if (bundle.gscBaselines) {
    fs26.writeFileSync(path27.join(dataDir, "gsc-baselines.json"), JSON.stringify(bundle.gscBaselines, null, 2));
