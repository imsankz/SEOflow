...
function resolvePythonScript(scriptName) {
  ...
  const candidates = [
    path17.join(process.cwd(), "python", scriptName),
    path17.resolve(here, "..", "python", scriptName),
    path17.resolve(here, "..", "..", "python", scriptName)
  ];
  return candidates.find((c) => fs18.existsSync(c)) || candidates[0];
}
function isUrl(s) {
...
function resolvePostFile(slug) {
  const postsDir = getPostsDir();
  const mdxPath = path20.join(postsDir, `${slug}.mdx`);
  if (fs20.existsSync(mdxPath)) return mdxPath;
  const mdPath = path20.join(postsDir, `${slug}.md`);
...
function saveBluf(result, dir = ".seoflow/bluf") {
  ...
  const mdPath = path20.join(dir, `${result.slug}.md`);
  const jsonPath = path20.join(dir, `${result.slug}.json`);
...
function findExtensionBundleRoot(extensionId, rootDir) {
  const candidates = [
    ...
    path21.resolve(path21.dirname(fileURLToPath2(import.meta.url)), "..", "extensions", extensionId),
    path21.resolve(path21.dirname(fileURLToPath2(import.meta.url)), "..", "..", "extensions", extensionId)
  ];
